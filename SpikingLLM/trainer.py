...
"""
# from spike_driven_quant.utils import set_quant_state, set_k_state, set_noise_state, set_error_state
import contextlib
import copy
import functools
import glob
...
class ZOTrainer(Trainer):
    ...
    def zo_step(self, model, inputs):
        ...
        self.delta.append((loss1-loss2).item())
...
class LOZOTrainer(Trainer):
    ...
    def lowrank_zo_step(self, model, inputs):
        ...
        self.projected_grad = ((loss1 - loss2) / (2 * self.args.zo_eps)).item()
        # self.projected_grad = ((loss1 - loss2) / (2 * 0.01)).item()
...
class SparseZOTrainer(Trainer):
    def _inner_training_loop(
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                # self.mask_median[name] = torch.abs(param.data).median()  
                self.mask_median[name] = torch.kthvalue(torch.abs(param.data).view(-1), max(int(args.ratio * param.data.view(-1).size(0)), 1))[0] 
    ...
    def zo_perturb_parameters(self, random_seed=None, scaling_factor=1):
        ...
        for name, param in self.named_parameters_to_optim:
            z = self.mask[name] * torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
            # z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
            param.data = param.data + scaling_factor * z * self.args.zo_eps
    ...
    def zo_step(self, model, inputs):
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            self.mask[name] = -torch.clip(torch.sign(torch.abs(param.data) - self.mask_median[name]), -1, 0)
        ...
        # self.proj_grad_small.append(self.projected_grad)


        # np.savetxt("proj_grad_small.txt", self.proj_grad_small)
        ...
        #     self.mask[name] = -torch.clip(torch.sign(torch.abs(param.data) - 1 * self.mask_median[name]), -1, 0)

            self.mask[name] = 1 

        logger.info(f"mask_num: {mask_num}") 
        # Sample the random seed for sampling z
        # self.zo_random_seed = np.random.randint(1000000000)
        ...
        self.proj_grad_vanilla.append(self.projected_grad)

        np.savetxt("proj_grad_vanilla.txt", self.proj_grad_vanilla)
        """
    ...
    def zo_update(self, model):
        ...
        # logger.info(f"self.projected_grad: {self.projected_grad}") 

        # self.projected_grad = np.clip(self.projected_grad, -10, 10)
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
            if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                param.data = param.data - self._get_learning_rate() * self.mask[name] * (self.projected_grad * z + args.weight_decay * param.data)
            else:
                param.data = param.data - self._get_learning_rate() * self.mask[name] * (self.projected_grad * z)
            
            # value_gz += torch.sum(self.mask[name].to("cpu") * param.grad.to("cpu") * z.to("cpu"))   
        
        # print("value_gz: ", value_gz) 
...
class HizooTrainer(Trainer):
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            zo_learning_rate = self._get_learning_rate()#zo_lr_scheduler(self.args.learning_rate, self.args.zo_lr_scheduler_type, self.args.warmup_step, self.args.decay_step, self.state.global_step, int(num_train_epochs))
    ...
    def sample_l2_ball(self, dim, r, dtype=torch.float32, device='cpu'):
        z = torch.randn(dim, dtype=torch.float32, device=device)
        
        direction = z / torch.norm(z, p=2)

        u = torch.rand(1, dtype=torch.float32, device=device).item()
        radius = r * (u ** (1.0 / dim))
        
        xi = (radius * direction).to(dtype)
        
        return xi



class ZOAdamWTrainer(Trainer):
    def _inner_training_loop(
        self, batch_size=None, args=None, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=None
    ):
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                self.fisher_matrix[name] = torch.zeros(size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
    ...
    def zo_perturb_parameters(self, random_seed=None, scaling_factor=1):
        ...
        for name, param in self.named_parameters_to_optim:
            z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
            fisher = torch.where(self.fisher_matrix[name] > 0, torch.rsqrt(self.fisher_matrix[name]), torch.zeros_like(self.fisher_matrix[name]))
            fisher = fisher / self.max *10.0
            # z = torch.sqrt(torch.Tensor([1.0])).to(param.data.device)*param.grad+torch.sqrt(torch.Tensor([1-1.0])).to(param.data.device)*z
            param.data = param.data + scaling_factor * fisher * z * self.args.zo_eps
    ...
    def zo_forward(self, model, inputs):
        ...
        with torch.inference_mode():
            inputs = self._prepare_inputs(inputs)
            # torch.manual_seed(self.noise_random_seed)
            with self.compute_loss_context_manager():
                loss = self.compute_loss(model, inputs)
                # if self.args.clear_activations:
                #     torch.cuda.empty_cache()
    ...
    def zo_step(self, model, inputs):
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                ...
                fisher_estimator = param.grad.data * param.grad.data
                self.fisher_matrix[name] = (args.fisher_smooth * self.fisher_matrix[name] +  (1-args.fisher_smooth)*fisher_estimator)

                self.max = max(self.max, torch.max(torch.where(self.fisher_matrix[name] > 0, torch.rsqrt(self.fisher_matrix[name]), torch.zeros_like(self.fisher_matrix[name]))).item())

    
        # # Sample the random seed for sampling z
        self.zo_random_seed = np.random.randint(1000000000)
        # # self.noise_random_seed = np.random.randint(1000000000)
        ...
        # # set_quant_state(model, weight_quant=False, act_quant=True)
        # # set_noise_state(model, False)
        # # loss4 = self.zo_forward(model, inputs)
    ...
    def zo_update(self, model):
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            # fisher_estimator = (self.projected_grad * z) * (self.projected_grad * z)
            ...
            fisher = fisher / self.max *10.0
            if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                # param.data = param.data - self._get_learning_rate() * (self.projected_grad *fisher* z + args.weight_decay * param.data)
                param.data = param.data - self._get_learning_rate() * (param.grad.data / (torch.sqrt(self.fisher_matrix[name]/(1-self.args.fisher_smooth**(self.state.global_step+1)))+1e-8) + args.weight_decay * param.data)
            else:
                # param.data = param.data - self._get_learning_rate() * (self.projected_grad *fisher* z)
                param.data = param.data - self._get_learning_rate() * (param.grad.data / (torch.sqrt(self.fisher_matrix[name]/(1-self.args.fisher_smooth**(self.state.global_step+1)))+1e-8))
            # param.grad = (self.projected_grad * z).detach()
