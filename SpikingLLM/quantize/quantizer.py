...
from phase_utils.hadamard_utils import random_hadamard_matrix
...
def quant_activation(x, scale, bits):
    ...
    qmin = -(2 ** (bits - 1))
    qmax = 2 ** (bits - 1) - 1
    scale = clamp_ste(scale,5e-4, 1e4)
...
class UniformAffineQuantizer(nn.Module):
    ...
    def static_fake_quant(self, x):
        ...
        v_0 = torch.ones_like(x_reshaped).mul(scale)*0.5
        if hasattr(self, 'initv'):
            v_0 = v_0 + torch.ones_like(x_reshaped[ 0, ...]) *  self.initv
        if round_zero_point is not None:
            # max_val = self.qmax * scale
            # min_val = torch.zeros_like(max_val)
            x_int = x_reshaped.add(round_zero_point.mul(scale))
            # x_int = torch.clamp(x_int, min_val, max_val)
            x_int = x_int.add(v_0.detach())
            x_int = floor_ste(x_int / scale)
        else:
            x_int = x_reshaped.add(v_0.detach())
            x_int = floor_ste(x_int / scale)
        ...
        # # x_int = floor_ste(x_reshaped / scale)
        # if round_zero_point is not None:
    ...
    def dynamic_fake_quant(self, x):
        ...
        if self.asym:
            ...
            quant_range = 2 * xmax
        scale = (quant_range / (2**self.n_bits-1)).clamp(min=5e-4, max=1e4)
