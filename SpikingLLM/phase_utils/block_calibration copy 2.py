...
def LMHT_params(slayer):
    params = []
    for module in slayer.modules():
        if isinstance(module, LMHTNeuron):
            for n, m in module.named_parameters():
                if (n.find('scale') > -1 or n.find('initv') > -1):
                    m.requires_grad = True
                    params.append(m)
    return iter(params)

def UniformAffineQuantizer_quant(slayer):
    params = []
    for module in slayer.modules():
        if not isinstance(module, LMHTNeuron):
            for n, m in module.named_parameters():
...
def UniformAffineQuantizer_weight(slayer):
    params = []
    for module in slayer.modules():
        for n, m in module.named_parameters():
...
def calibration(
    ...
    if hasattr(spike_model.model, 'rotary_emb'):
        # for llama-3.1
        spike_model.model.rotary_emb = spike_model.model.rotary_emb.to(dev)
    ...
    slayers[0] = Catcher(slayers[0], fp_train_inps)
    ...
    with torch.no_grad():
        for i in range(iters):
            ...
            try:
                ...
                pass
    position_ids = slayers[0].position_ids
    attention_mask = slayers[0].attention_mask
    ...
    slayers[0] = Catcher(slayers[0],fp_val_inps)
    ...
    spike_train_inps = copy_block_dataset(fp_train_inps)
    spike_val_inps = copy_block_dataset(fp_val_inps)
    ...
    try:
        ...
        for block_index in range(len(slayers)):
            logger.info(f"=== Start calibration blocks {block_index}===")
            layer = slayers[block_index].to(dev)
            layer.to(dev)
            ...
            if args.epochs > 0:
                ...
                set_quant_parameters(layer, args.quant_lr > 0)
                set_weight_parameters(layer, args.weight_lr > 0)
                param = []
                if args.quant_lr > 0:
                    param.append({"params": quant_parameters(layer), "lr": args.quant_lr})
                if args.weight_lr > 0:
                ...
                # if args.quant_lr > 0:
                #     param.append({"params": UniformAffineQuantizer_quant(slayer), "lr": args.quant_lr})
                # if args.weight_lr > 0:
                #     param.append({"params": UniformAffineQuantizer_weight(slayer), "lr": args.weight_lr})
                ...
                for epoch in range(args.epochs):
                    ...
                    train_loss, gradient_norm = train_one_epoch(layer, spike_prefixed_key_values, attention_mask, position_ids,
                    ...
                    val_loss = eval_one_epoch(layer, spike_prefixed_key_values, attention_mask, position_ids,
                        loss_func, dev, traincast,
            ...
            if args.epochs>0 or args.mse_init:
                ...
                spike_update_dataset(slayer, spike_train_inps, spike_train_inps, dev, attention_mask, position_ids, spike_prefixed_key_values, args.T)
                spike_update_dataset(slayer, spike_val_inps, spike_val_inps, dev, attention_mask, position_ids, spike_prefixed_key_values, args.T)
    ...
    torch.cuda.empty_cache()
    gc.collect()                    
    spike_model.config.use_cache = use_cache
    return spike_model
