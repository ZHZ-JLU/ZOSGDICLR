...
def spike_get_kv_cache(T, prefixed_key_values, bs=1):
    from phase_utils.snn_utils import replicate_past_key_values
    if bs > 1:
        prefixed_key_values = kv_cache_repeat(prefixed_key_values, bs)
    prefixed_key_values = replicate_past_key_values(prefixed_key_values, T)
    if prefixed_key_values is not None:
        kv_cache = DynamicCache.from_legacy_cache(prefixed_key_values)
    else:
        kv_cache = None
...
class WrappedPrefixCausalLM(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        if kwargs.get("past_key_values") is None:
            ...
            self.prefixed_key_values = mv_kv_cache(self.prefixed_key_values, self.model)
            self.bs_prefixed_key_values = kv_cache_repeat(self.prefixed_key_values, bs)
            kwargs["past_key_values"] = DynamicCache.from_legacy_cache(self.bs_prefixed_key_values)
            if kwargs.get("attention_mask") is not None:
                kwargs["original_attn_mask"] = copy.deepcopy(kwargs["attention_mask"])
