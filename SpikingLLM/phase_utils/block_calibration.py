    ...
    try:
        ...
        for block_index in range(len(layers)):
            ...
            if args.epochs > 0:
                ...
                assert args.quant_lr >= 0 or args.weight_lr >= 0 or args.neuron_lr >= 0
                set_quant_parameters(slayer, args.quant_lr > 0)
                # set_lmht_parameters(slayer, args.quant_lr > 0)
                set_weight_parameters(slayer, args.weight_lr > 0)
                param = []
                if args.quant_lr > 0:
                    param.append({"params":quant_parameters(slayer),"lr":args.quant_lr})
                if args.weight_lr > 0:
                    param.append({"params":weight_parameters(slayer),"lr":args.weight_lr})
                if args.neuron_lr > 0:
                ...
                trainable_number = trainable_parameters_num(slayer)
                ...
                for epoch in range(args.epochs):
                    ...
                    train_loss, gradient_norm = train_one_epoch(slayer, spike_prefixed_key_values, attention_mask_s, position_ids,
