...
class FSNeuron(nn.Module):
    def __init__(self, T: int, quantized_shape, quantized_item_stat, num_grains: int = 1, beta : float = 10.0, genotype : list = None, neuron_d : torch.Tensor = None, tau : float = 1.0, neuron_h : torch.Tensor = None, neuron_theta : torch.Tensor = None, neuron_v0 : bool = False, fast : bool = True):
        """
        Args:
        """
        super().__init__()
        ...
        self.group_size = quantized_shape[-1]
        self.tau = nn.Parameter(torch.zeros((1,1))) #None
        self.beta = beta
        ...
        # if neuron_v0:
        #     self.v0 =  nn.Parameter(torch.ones((1,1)) * self.tau * 2**(-self.T-1))
        # else:
        #     self.v0 =  nn.Parameter(torch.zeros((1,1)))
        
        self.B = nn.Parameter(torch.ones((1,1))*2.0)
        self.theta = nn.Parameter(torch.ones((1,1))*2.0)
        self.h = nn.Parameter(torch.ones((1,1))*2.0)
    ...
    def get_phase_power(self, t: int, mode):
        if mode == 1:
            return torch.pow(self.B, -(t+1)) * self.tau
        if mode == 0:
            return torch.pow(self.theta, -(t+1)) * self.tau
        if mode == -1:
            return torch.pow(self.h, -(t+1)) * self.tau
    
    def forward(self, x):
        T, bs, n, dim1 = x.shape
        # y = []

        # self.find_activation_quant_param(x)
        ...
        #     self.v = torch.abs(self.v) #+ self.v0

        #     spikes = []
        #     for t in range(self.T):
        #         h = self.get_phase_power(t, -1)
        #         theta = self.get_phase_power(t, 0)
        ...
        # for t in range(self.T):
        #     d = self.get_phase_power(t, 1)
        #     y.append(v_sign * d * spikes[t])       

        # y = torch.stack(y, dim=0).reshape(self.T, bs, n, dim1)
        
        if self.fast:
            # y = (((self.v / (self.tau * 2.0**(-self.T))).round().clamp(0, 2**(self.T)-1)*(self.tau * 2.0**(-self.T)))*v_sign).unsqueeze(0)
            y = ((round_ste(torch.abs(x) / (self.tau * 2.0**(-self.T))).clamp(0, 2**(self.T)-1)*(self.tau * 2.0**(-self.T)))*torch.sign(x))
    ...
    def find_activation_quant_param(self, quantized_item_stat):
        ...
        # del self.tau
        self.tau = nn.Parameter(torch.ones((1,1))*float(xmax))
...
class LMHTNeuron(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.asym:   #
            ...
            self.v = torch.ones_like(x_reshaped[ 0, ...]) * scale * self.initv  # 
            #
            ...
            T, bs, n, dim1 = x.shape
            x_reshaped = x.reshape(T, bs, n, -1, self.group_size)
            self.v = torch.ones_like(x_reshaped[ 0, ...]).mul(scale)*self.initv  # 
            #
            # self.v = torch.ones_like(x_reshaped[ 0, ...]) * scale * 0.5
            # self.v = self.v + torch.ones_like(x_reshaped[ 0, ...]) *  self.initv
