...
from phase_utils.quant_utils import set_op_by_name
...
def set_phase_model_time_step(model, T, logger, fast):
    for name, module in model.named_modules():
        if fast:
            if hasattr(module, "T") and isinstance(module, FSNeuron):
                module.T = T
            elif hasattr(module, "T"):
                module.T = 1
        else:
            if hasattr(module, "T"):
                module.T = T
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, phaseSnnLinear):
            ...
            if neuron_parameter is not None:
                ...
                module.input_quantizer = FSNeuron(T=args.T, quantized_shape=quantized_shape, quantized_item_stat=input_stat, fast=args.fast, neuron_v0=False)
            ...
            if neuron_parameter is not None:
                ...
                module.output_quantizer = FSNeuron(T=args.T, quantized_shape=quantized_shape, quantized_item_stat=output_stat, fast=args.fast, neuron_v0=False)
    ...
    for name, module in model.named_modules():
        if isinstance(module,phaseSnnIdentity) and ('softmax_Identity' in name or 'q_Identity' in name):
            ...
            if neuron_parameter is not None:
                ...
                if 'softmax_Identity' in name:
                    module.input_quantizer = FSNeuron(T=args.T, quantized_shape=quantized_shape, quantized_item_stat=output_stat, fast=args.fast, neuron_v0=True)
                else:
                    module.input_quantizer = FSNeuron(T=args.T, quantized_shape=quantized_shape, quantized_item_stat=output_stat, fast=args.fast, neuron_v0=False)
                # if ('softmax_Identity' in name):
                #     tau = module.input_quantizer.tau
            ...
            if neuron_parameter is not None:
                if 'silu_Identity' in name or 'linear_Identity' in name:
                    num_grains, genotype, neuron_d, tau, neuron_h, neuron_theta, neuron_v0 = neuron_parameter[f'{name}.input']
                    module.input_quantizer = FSNeuron(T=args.T, quantized_shape=quantized_shape, quantized_item_stat=output_stat, num_grains=num_grains, genotype=genotype, neuron_d=neuron_d, tau=tau, neuron_h=neuron_h, neuron_theta=neuron_theta, neuron_v0=neuron_v0)
                else:
                    num_grains, genotype, neuron_d, tau, neuron_h, neuron_theta = neuron_parameter[f'{name}.input']
...
def change_T(model, T):
    for m in model.modules():
        # if isinstance(m, QuantLinear):
        if isinstance(m, (FSNeuron)):
            m.change_T(T)