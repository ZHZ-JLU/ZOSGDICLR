...
import math
from transformers.models.llama.modeling_llama import LlamaPreTrainedModel, LlamaDecoderLayer
...
from SNN.spike_layer import seq_matmul, compute_hadamard_v2, compute_hadamard_v3
from phase.phase_neuron import FSNeuron
from transformers.utils.deprecation import deprecate_kwarg

class phaseSnnLinear2(nn.Module):
    """
...
class phaseSnnLinear(nn.Module):
    """
    Quantized Module that can perform quantized convolution or normal convolution.
    To activate quantization, please use set_quant_state function.
    """
    def __init__(
        ...
        # self.input_quantizer = FSNeuron(T=T)
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.use_act_quant and hasattr(self, "input_quantizer"):
            input = self.input_quantizer(input)

        out = self.fwd_func(input, weight, bias, **self.fwd_kwargs)

        if self.use_act_quant and hasattr(self, "output_quantizer"):
...
class phaseSnnIdentity(nn.Module):
    def __init__(
        self,
        out_features,
        T,
...
class phaseSnnSdpaLlamaAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, org_module, config, T, args=None):
        super().__init__()
        self.config = config
        ...
        self.head_dim = org_module.head_dim #self.hidden_size // self.num_heads
        ...
        self.k_proj = phaseSnnLinear(
            org_module.k_proj, T
        )
        self.v_proj = phaseSnnLinear(
            org_module.v_proj, T
        )
        self.q_proj = phaseSnnLinear(
            org_module.q_proj, T
        )
        self.o_proj = phaseSnnLinear(
            org_module.o_proj, T
        )
        self.flag = True
        if self.flag:
            self.q_Identity = phaseSnnIdentity(org_module.q_proj.out_features, T)
            self.k_Identity = phaseSnnIdentity(org_module.q_proj.out_features,T)
            self.v_Identity = phaseSnnIdentity(org_module.q_proj.out_features, T)
            self.softmax_Identity = phaseSnnIdentity(self.num_heads, T)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if position_embeddings is not None:
            ...
            cos, sin = self.rotary_emb(value_states, position_ids=kwargs["position_ids"])
            query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, kwargs["position_ids"])
        ...
        if attention_mask is not None:
            ...
            if kwargs.get("original_attn_mask") is not None:
                assert bsz>1
                mask = torch.tril(kwargs['original_attn_mask'].unsqueeze(1).repeat(1, q_len, 1)).bool()
                extended_mask = mask.any(dim=-1, keepdim=True).repeat(1, 1, key_states.shape[-2]-q_len)
                causal_mask = torch.cat([extended_mask, mask], dim=-1).unsqueeze(1)
        ...
        attn_scores = torch.max(attn_scores, torch.tensor(torch.finfo(attn_scores.dtype).min))
...
class phaseSnnLlamaMLP(nn.Module):
    def __init__(self, org_module, config, T):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size
        self.gate_proj = phaseSnnLinear(
            org_module.gate_proj, T
        ) 
        self.up_proj = phaseSnnLinear(
            org_module.up_proj, T
        ) 
        self.down_proj = phaseSnnLinear(
            org_module.down_proj, T
        ) 
        self.act_fn = ACT2FN[config.hidden_act]
        self.T = T

        self.linear_Identity = phaseSnnIdentity(self.gate_proj.out_features, T)
        self.silu_Identity = phaseSnnIdentity(self.gate_proj.out_features, T)
    ...
    def forward(self, x):
        ...
        if self.config.pretraining_tp > 1:
            ...
            out_up = self.linear_Identity(out_up)
            ...
            act_out_g = self.silu_Identity(act_out_g)
...
class phaseSnnRMSNorm(nn.Module):
    def __init__(self, 
                ori_norm,
                T=2,
                ):
        super().__init__()
        self.register_parameter('weight',ori_norm.weight)
        self.bias = None
        ...
        self.T = T
        # self.output_quantizer = FSNeuron(T=T)
...
class phaseSnnEmbedding(nn.Module):
    def __init__(
        self,
        original_embedding,
        T=4,
        ...
        self.register_parameter('weight', nn.Parameter(self.oriebd.weight.clone().detach()))
