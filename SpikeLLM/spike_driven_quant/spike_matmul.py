...
class SpikeQuantMatMul(nn.Module):
    ...
    def static(self):
        ...
        del self.mask_low_mse
        self.register_parameter(f"mask_low_mse",torch.nn.Parameter(mask_low_mse, requires_grad=False))
        del self.sum
    ...
    def forward(self, double):
        ...
        # if self.matmul_func == torch.bmm:
        #     x1 = x1.reshape(-1, x1.shape[2], x1.shape[3])
        #     x2 = x2.reshape(-1, x2.shape[2], x2.shape[3])
